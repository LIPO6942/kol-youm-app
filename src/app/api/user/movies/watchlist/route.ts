...
export async function POST(req: NextRequest) {
  try {
    ...
    const fieldName = type === 'movie' ? 'moviesToWatch' : 'seriesToWatch';
    ...
    return NextResponse.json({
      success: true,
      message: `${type === 'movie' ? 'Film' : 'Série'} "${title}" ajouté à la liste à voir`
    });

  } catch (error: any) {
    console.error('Erreur lors de l\'ajout à voir:', error);
    return NextResponse.json(
